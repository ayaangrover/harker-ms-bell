    ...
    <v-app-bar app color="primary" elevate-on-scroll style="overflow-x: auto; opacity: 0.9;">
      ...
      <transition name="fade" mode="out-in">
        <v-toolbar-title v-if="calendar.titleChanging" key="changing" class="title text-center"
                         :style="{minWidth: $vuetify.breakpoint.smAndUp ? '235px' : '110px'}"
        >
        ...
        <v-toolbar-title v-else id="title" key="title" class="headline text-center"
                         :style="{minWidth: $vuetify.breakpoint.smAndUp ? '235px' : '110px'}" ga-on="click"
                         ga-event-category="title" ga-event-action="click" @click="changeTitle"
        >
          <span v-if="$vuetify.breakpoint.smAndUp">Harker </span>MS Bell<span v-if="$vuetify.breakpoint.smAndUp"> Schedule</span>
        </v-toolbar-title>
      </transition>
  ...
  data() {
    return {
      ...
      settings: {
        ...
        periodColors: JSON.parse(localStorage.getItem("periodColors")) || ["blue2", "red2", "green2", "yellow2", "orange2", "teal2", "purple2", "indigo2"],
